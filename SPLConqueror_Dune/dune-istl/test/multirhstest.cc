...
void test_all_solvers(std::string precName, Operator & op, Prec & prec, unsigned int N, unsigned int Runs)
{
  ...
  int verb = 1;
  //VARIATIONPOINT(SOLVER; solver; Dune::LoopSolver<Vector>;)
...
void test_all(unsigned int Runs = 1)
{
  ...
  //VARIATIONPOINT(PRECON; precon; Dune::SeqJac<Matrix,Vector,Vector,l>;)
  ...
  Dune::SeqILU<Matrix,V.ector,Vector> ilu_1(A,1,0.1);     // preconditioner object
